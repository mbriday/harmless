...
# Example :
# If GCC is in ~/x-tools/arm-none-eabi/bin/arm-none-eabi-gcc
# set ARM_XTOOLS to "~/x-tools/arm-none-eabi/bin/arm-none-eabi-"
# you may put
# "export ARM_XTOOLS=~/x-tools/arm-none-eabi/bin/arm-none-eabi-"
...
try:
	...
	print "The ARM_XTOOLS environnement variable doesn't exist...",
	print "Please define it: export ARM_XTOOLS=path/to/cross-compiler/powerpc-elf-"
	sys.exit()
...
def handle_s(fn):
	...
	return os.system(xgcc + " " + fn + " -o "+elf+" -g -Wl,-n,-Tinternal/script.ld,"+buildDir+"crt0.o,-Map,"+mapFile+" -nostdlib -mfloat-abi=soft -mcpu=arm7tdmi")

# Compile the given ".c" file
def handle_c(fn):
	elf=buildFileName(fn,'elf')
	mapFile=buildFileName(fn,'map')
	return os.system(xgcc + " " + fn + " -o "+elf+" -g -Wl,-n,-Tinternal/script.ld,"+buildDir+"crt0.o,-Map,"+mapFile+" -nostdlib -mfloat-abi=soft -lm -lc -lgcc -mcpu=arm7tdmi")
...
for test_file in test_files:
	...
	if not os.path.isfile(buildDir+"crt0.o") :
		os.system(xas + " internal/crt0.s -o "+buildDir+"crt0.o -mfloat-abi=soft")
	...
	os.system("cd internal && ./trace_sim_gdb.sh ../"+elf+" "+str(inst)+" > ../"+outputFile+" && cd ..")
	print "."
	sys.stdout.flush()
# End
