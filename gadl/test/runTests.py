...
#for Mac OS 10.9. The simulator is not build with the /usr/bin/python version :-/
...
def readConfigFile(filename):
	...
	if len(config['files']) != 0 and len(config['locals']) != 0:
		...
		print " *OR*  a list of files in examples/ (using 'files=')"
		config['files'] = []
...
def buildSimSources(scriptWorkingDir,configList):
	...
	-> exits if there is a compilation error of description in examples/
	...
	#Eliminate files that appear several times in configs (for global files in examples/
	fileSet=set()
	...
	print "there are ",len(fileSet)," simulator sources from examples/ to generate (gadl)."
	...
	for process,file in zip(processList,fileSet): #global files (in examples/) should not generate an error.
		process.wait()
...
def launchTestsOnTarget(scriptWorkingDir,configList,modelName,target,simDir):
	...
	for config in configList:
		for script in config['customScript']:
			...
			cmd=pythonInterpreter+' '+script+' -p '+simDir+' -n '+modelName
