...
SIMULATOR_PATH="../../../examples/avr/AT90CAN128/"		# Simulator folder
RESULT_PATH="bench_result.txt"		# Target for result file
EXECS_PATH="./bench_exec"			# ELF Binary .elf & .stack config files folder
NB_INSTR_SIMULATION=1000000000		# How long simulation will execute
...
## PATCHING SOURCES ##
def patchSources():
	command_line="sed \"s/sram_getChunk/mem_getChunk/g\" stackObserver.cpp > temp"
	args=shlex.split(command_line)
	p = subprocess.Popen(args,bufsize=-1,close_fds=True,shell=False)
	p.wait()	# Wait the end
	command_line="mv temp stackObserver.cpp"
	args=shlex.split(command_line)
	p = subprocess.Popen(args,bufsize=-1,close_fds=True,shell=False)
	p.wait()	# Wait the end
...
def compileSimulator(stringOptions):
	...
	# BUILD
	command_line="make -j2 -C "+simPath+" "+"DEFINES+=-DHOST_IS_LITTLE_ENDIAN  DEFINES+=-DUSE_LIBELF LIBELF=1 DEFINES+=-DHOST64 "+stringOptions
	args=shlex.split(command_line)
	p = subprocess.Popen(args,bufsize=-1,close_fds=True,shell=False)
...
#tc1=time.time()
compileSimulator("DEFINES+=-DGADL_NO_ACTION")
#ts1=time.time()
...
#tc3=time.time()
compileSimulator("DEFINES+=-DGADL_NO_ACTION DEFINES+=-DGADL_SP_CHECK_ALLOWED DEFINES-=-DGADL_SP_FAILURE_CHECK_ALLOWED")
#ts3=time.time()
