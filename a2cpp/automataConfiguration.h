 ...
 * $Rev: 7 $
 * $Author: mik $
 * $URL: https://a2cpp.rts-software.org/svn/automataConfiguration.h $
...
class automataConfiguration 
{
	...
	std::map<std::string, unsigned int> m_instructionClassesStringMap; 
	...
	void readInstructionClasses(std::istream &file);
	...
	//including '-'
	unsigned int const getNbInstructionClasses() {return m_instructionClassesStringMap.size();};
	...
	//std::vector<std::string> &getInstructionClassesStringVector() {return m_instructionClassesStringVector;};
...
//	std::vector<automataEdge *> &getAutomataEdgesVector() {return m_automataEdgesVector;};
	...
	//return the number of bits required to code one pipeline stage, i.e. nb of bits to encode instruction classes.
