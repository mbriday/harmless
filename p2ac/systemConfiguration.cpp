...
//#include "dataDependencyInstruction.h"
...
systemConfiguration::~systemConfiguration()
{
	...
	const unsigned int sizeInst = m_instructionVector.size();
	for(unsigned int i=0; i<sizeInst; ++i) macroMyDelete (m_instructionVector[i]);
...
void systemConfiguration::processSymbolPipeline(ifstream &file)
{
	...
	if(command == "addStage") 
	{
		if(m_NotificationDefined) 
			cerr << "internal error: no instruction should be defined after a notification!" << endl;
		string name;
		file >> name;
	...
	} else if(command == "addInstructionClass")
	{
		...
		instruction *inst = pipe->addInstructionClass(name);
...
//	else if(command == "addDataDependencyInstruction")
//	{
//		string name;
//		file >> name;
//		//create data dep instruction name
//		m_DapaDependencyInstructionNameVector.push_back(name);
//		//create and add to the pipeline.
//		dataDependencyInstruction *ddc = new dataDependencyInstruction();
//		m_DapaDependencyInstructionVector.push_back(ddc);
//#ifdef DEBUG_LOAD_CONFIG
//		cout << "add data dependency instruction " << name
//		<< " to " << m_pipelineName << endl;
//#endif
//	}
}

void systemConfiguration::processSymbolInstruction(int index, ifstream &file)
{
	   ...
	   (command == "releaseResource"))
	{
		...
		m_instructionVector[index]->resourceAccess(type,
    ...
    } else if(command == "stay")
	{
		...
		assert(val>1);
		m_instructionVector[index]->instructionHaveExtraCyclesInStage(m_pipelineStageDescriptorVector[indexPS], val-1);
		
		//m_instructionVector[index]->StayInStage(m_pipelineStageDescriptorVector[indexPS], val);
		#ifdef DEBUG_LOAD_CONFIG
...
//void systemConfiguration::processSymbolDapaDependencyInstruction(int index, ifstream &file)
//{
//	string symbol = m_DapaDependencyInstructionNameVector[index];
//	string  command;
//	string ps;
//	file >> command >> ps;
//	const int indexPS = getIndexInVector(m_pipelineStageDescriptorNameVector, ps);
//	if(command == "required") {
//		m_DapaDependencyInstructionVector[index]->requiredStageVector.push_back(indexPS);
//        #ifdef DEBUG_LOAD_CONFIG
//			cout << "data dep instruction class " << symbol << " requires "
//			     << "data dep access at stage " << ps << endl;
//		#endif
//	} else if(command == "lock") {
//		m_DapaDependencyInstructionVector[index]->lockStageVector.push_back(indexPS);
//		#ifdef DEBUG_LOAD_CONFIG
//			cout << "data dep instruction " << symbol << " locks "
//			     << "data dep access at stage " << ps << endl;
//		#endif
//		
//	} else if(command == "unlock") {
//		m_DapaDependencyInstructionVector[index]->unlockStageVector.push_back(indexPS);
//		#ifdef DEBUG_LOAD_CONFIG
//			cout << "data dep instruction " << symbol << " unlocks "
//			     << "data dep access at stage " << ps << endl;
//		#endif
//	} else {
//		cerr << "unrecognized command for data dependency instruction: " << symbol << endl;
//	}
//}
...
void systemConfiguration::processSymbol(string &symbol, ifstream &file)
{
	...
	else if((index = getIndexInVector(m_instructionClassNameVector, symbol)) != m_instructionClassNameVector.size())
	{
		...
		processSymbolInstruction(index,file);
	}
}
...
bool systemConfiguration::load(const string filename)
{
	...
	if(verbose)
	{
		cerr << "load configuration from file: " << filename << endl;
	}
	ifstream file;
	...
	case 8:
		...
		if(command != "model")
		{
			cerr << "a model name should be given!" << endl;
		}
		file >> m_modelName;
		file >> command;
		while(!(file.eof()))
...
bool systemConfiguration::processCommand(const string command, ifstream &file)
{
	...
	else if(command == "pipeline") 
	{
		...
		//In order to force the first resource to manage branching.
		resourceDescriptor *resDesc = NULL ;
		macroMyNew (resDesc, resourceDescriptor("branch",1, true));
		m_resourceDescriptorNameVector.push_back("branch");
		m_resourceDescriptorVector.push_back(resDesc);
		m_pipeline->addResourceDescriptor(resDesc);
	}
