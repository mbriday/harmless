...
//cause p2ac to be verbose: showing septs of data generation.
bool verbose;

static void print_help()
{
	cout << "p2ac is copyright (c) IRCCyN 2005+" << endl;
	cout << "p2ac est protege par la loi sur la propriete intellectuelle" << endl;
	cout <<  endl;
	...
	cout << endl;
	cout << "Usage: p2ac [options] file1 file2" << endl;
	cout << "The file1 is the configuration file." << endl;
	cout << "The file2 is the analysed program." << endl;
	cout << "Options:" << endl;
	cout << "--help\t\tthis help" << endl;
	cout << "--output|-o file\toutput file (.ac2ac)." << endl;
	cout << "--graphviz|-gv file\texport file in GraphViz format (.dot)." << endl;
	cout << "--verbose|-v \tcause p2ac to be verbose: showing septs of data generation (to stderr)" << endl;
	//cout << "--info-bdd|-i \tcause p2ac to display the BDD package information messages" << endl;
	//cout << "--max-ram-size=n|-m=n \tcause p2ac to constraint the BDD memory usage < n MB" << endl;
	//cout << "--max-and-cache-size=n|-a=n \tcause p2ac to constraint the BDD AND cache map size to < 2**n" << endl;
	//cout << "--max-ops-cache-size=n|-b=n \tcause p2ac to constraint the BDD single operand operation cache map size to < 2**n" << endl;
	//cout << "--max-hash-map-size=n|-c=n \tcause p2ac to constraint the BDD hash map size to < 2**n" << endl;
}
...
static bool parseArgumentList(int argc, const char *argv[])
{
	...
	while(i < argc)
	{
				  ...
				  (!strncmp(argv[i],"-gv",3)))
		{
			...
			if((i+1) < argc)
			{
				graphvizFilename = argv[i+1];
			}
			i+=2;
		} else if(!configFilenameRead)
		{
			//the filename is the config file.
			configFilename = argv[i];
			i++;
			configFilenameRead = true;
		} else
		{
			//the filename is the analysed program.
...
int mainForLIBPM (int argc, const char * argv[])
{
	...
	if(!parseArgumentList(argc, argv)) {
		if(configFilename.empty()) {
			...
			if(verbose) {
				autom->printInternalConstants();
			}
			if(verbose) {
				cerr << endl << "Computing automata transitions... " << flush;
			}
			C_Timer timer;
			//We compute the automata according to the program.
			autom->computeAutomata(program);
			if(verbose) {
				co << cStringWithUnsigned (autom->transitionCount ()) << " transitions ("<< timer << ")\n";
			}
			//autom->translateTransitionArrayIntoBDD (verbose) ;
			if(verbose) {
				autom->printInternalStats();
			}
			if(!graphvizFilename.empty()) {
				autom->exportGraphViz(graphvizFilename);
			}
			/* No memorisation for the moment!
			...
			if(!graphvizFilename.empty()) {
				autom->exportGraphViz(mini); }
			if(verbose) {
				autom->printInternalStats(); }
			//save AFTER minimization ..*/
			if(!outputFilename.empty()) {
				autom->save(outputFilename, config.modelName());
			}
			macroMyDelete (autom) ;
			/*if (verbose) {
