...
instructionClass::instructionClass(const std::string inName, pipeline *pipe, const unsigned int id)
...
void instructionClass::resourceAccess(const resourceAccessType type, resourceDescriptor *res, pipelineStageDescriptor *ps, const unsigned int nbAccess)
{
	SEGFAULTIF(type>2);
...
std::vector<resourceDescriptorAccess> &instructionClass::getResourceDescriptorVector(
...
std::string const instructionClass::debug()
{
	string result;
...
void instructionClass::instructionHaveExtraCyclesInStage(pipelineStageDescriptor *psDesc, const unsigned int nbCycle)
{
...
unsigned int instructionClass::getNbExtraCyclesFor(pipelineStageDescriptor *psDesc)
{
	const unsigned int size = m_nbExtraCyclesVector.size();
	for(unsigned int i=0; i < size; i++)
